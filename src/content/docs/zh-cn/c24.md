...
:::tip[本章内容]
- 什么是 vocabulary types
- 如何使用 `variant` 和 `any` 数据类型
- 什么是 `tuple`，以及如何使用它
- 如何在 `optional` 上使用 monadic operations
...
vocabulary types 是一类你很可能会高频使用的类型，使用频率几乎不亚于 `int`、`double` 这类基础类型。它们经常被拿来构建更复杂的类型。使用 vocabulary types，可以让代码更安全、更高效，也更容易编写、阅读和维护。本书前面已经讨论过的一些 vocabulary types 包括 `vector`、`optional`、`string`、`unique_ptr`、`shared_ptr` 等。

本章先从另外两种 vocabulary types 开始：`variant` 和 `any`。接着会更深入地讨论 `tuple`，它可以看作 `pair` 的泛化，以及它所支持的各种操作。随后会介绍 `optional` 对 monadic operations 的支持，这让你在 `optional` 上链式组合操作时轻松很多，因为你不需要在每一步之前都先判断该 `optional` 是否为空。本章最后会介绍 `expected`。这是一种既可以保存某个期望类型的值，也可以保存错误值的数据类型；其中用于表示错误的类型，可以与表示正常值的类型完全不同。
