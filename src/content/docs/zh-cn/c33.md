...
:::tip[本章内容]
- 什么是 pattern，以及它与 design technique 的区别
- 如何使用以下 pattern：
- Strategy
- Abstract Factory
- Factory Method
...
*Design pattern* 是一种解决通用问题的标准化程序组织方式。与 technique 相比，design pattern 的语言依赖性通常更弱。pattern 和 technique 之间的边界本来就有些模糊，不同书籍对它们的定义也不完全一致。本书将 technique 定义为“更偏 C++ 语言本身的策略”，而 pattern 则是适用于任何 object-oriented language（例如 C++、C#、Java 或 Smalltalk）的更一般性的 object-oriented design 策略。实际上，如果你熟悉 C# 或 Java，读本章时你会认出其中很多模式。

design pattern 有名字，而这是一项巨大优势。名字本身就携带含义，因此能帮助人们更容易交流解决方案；同时，pattern 的名字也能帮助开发者更快理解某个设计。不过，也要注意：某些 pattern 其实有多个名字，而不同资料对某些 pattern 的区分方式也略有模糊，描述与归类方式并不总是完全一致。事实上，不同书和不同资料中，你甚至可能看到同一个名字被用于不同 pattern。至于究竟哪些设计方法才算 pattern，本身也存在争议。除少数例外外，本书主要遵循那本经典著作 *Design Patterns: Elements of Reusable Object-Oriented Software*（Erich Gamma 等著，Addison-Wesley Professional, 1994）中的术语体系。若有必要，本章也会注明其他 pattern 名称或变体。

design pattern 这个概念本身非常简单，但威力很强。一旦你开始能够识别程序中反复出现的 object-oriented interaction，想要找到一种优雅解法，往往就会变成“该选哪一个合适的 pattern”这个问题。

市面上已有整本书专门讲 design pattern，因此本章只会相对详细地挑选其中一小部分更重要的 pattern，并给出示例实现。这已经足以帮助你建立对 design pattern 的整体感受。

任何设计相关话题都很容易引发程序员之间的争论，而我认为这是一件好事。不要把这些 pattern 当成“完成某件任务的唯一方式”；更好的做法，是从它们的方法与思想中吸收可复用部分，再据此继续打磨它们，甚至形成新的 pattern。

## Strategy Pattern

*Strategy pattern* 是支持 *dependency inversion principle*（DIP）的一种方式；相关讨论见[第 6 章](../c06)“面向复用设计”。在这个 pattern 中，interface 被用来反转依赖关系。对每一项对外提供的服务，都应建立一份 interface；当某个 component 需要一组服务时，就把这些服务的 interface 注入给这个 component，这种机制叫 *dependency injection*。Strategy pattern 的一个直接好处，是它会让 unit testing 更容易，因为你可以很方便地把这些服务 mock 掉。本节将以一个 logging mechanism 为例，展示如何用 Strategy pattern 来实现。

### 示例：Logging 机制

这个基于 Strategy pattern 的 logger，会使用一个 interface（也就是 abstract base class）叫 `ILogger`。任何想写日志的代码，都只依赖这个 `ILogger` interface。随后，一个该 interface 的 concrete 实现会被注入到任何需要 logging 能力的代码中。采用这种 pattern 后，unit test 就可以很容易地注入一个专门的 mock 实现，用来验证“是否记录了正确的信息”。它最大的优势，是 concrete logger 可以在不修改任何 library code 的前提下被轻松替换；client code 只需要传入自己想使用的 logger 即可。

### Strategy-Based Logger 的实现
