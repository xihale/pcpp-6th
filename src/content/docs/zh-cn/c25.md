...
:::tip[本章内容]
- 什么是分配器 (allocator)
- 如何编写符合标准库 (Standard Library) 约定的自定义算法 (algorithm)、容器 (container) 和迭代器 (iterator)
:::
...
:::note
本书通常约定：函数和成员函数名不使用下划线，且每个单词首字母大写，但第一个单词除外，例如 `getIndex()`。不过，本章讨论的是如何扩展 Standard Library，因此这里会采用 Standard Library 所使用的命名风格：函数和成员函数名全部小写，并以下划线分隔单词，例如 `get_index()`。类名同样也遵循 Standard Library 的命名约定。
:::
