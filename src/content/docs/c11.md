...
|`#if [expression] #elif [expression] #else #endif` | Conditionally include a block of code based on the result of a given expression. | Often used to provide specific code for specific platforms. |
...
| <cpp23></cpp23>`#elifdef [id] #elifndef [id]` | `#elifdef [id]` is equivalent to `#elif defined(id)` and `#elifndef [id]` is equivalent to  `#elif !defined(id)`. | Shorthand notations for other functionality. |
